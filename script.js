...
function getProducerById(data, producerId) {
  // your code here
  return data.producers.find(producerId === producer.id);
}

function canAffordProducer(data, producerId) {
  // your code here
  return getProducerById(data,producerId).price
}
...
function buyButtonClick(event, data) {
  ...
  if ( event.target.tagName === 'BUTTON') {
    const producerId = event.target.id.slice(6);
    const result = attemptToBuyProducer(data,producerId);
    if (renderProducers(data)) {
      updateCoffeeView(data.coffee);
